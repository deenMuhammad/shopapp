...
query getSaleProductBatchByCategory($pageSize: Int!, $next: Int!, $category: Int!) {
    getSaleProductBatchByCategory(pageSize: $pageSize, next: $next, category: $category) {
...
export default withApollo(class SaleCategory extends React.Component {
    ...
    getProducts = (id) => {
        this.state.hasMore && this.props.client.query({
            ...
            .then(res => {
                if (this.state.next === 1 && res.data.getSaleProductBatchByCategory.products.length === 0) {
                    this.setState({ empty: true })
                } else {
                    this.setState({
                        next: res.data.getSaleProductBatchByCategory.next,
                        hasMore: res.data.getSaleProductBatchByCategory.hasMore,
                        products: [...this.state.products, ...res.data.getSaleProductBatchByCategory.products],
