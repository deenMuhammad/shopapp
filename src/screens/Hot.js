...
const HOT_PRODUCTS_QUERY = gql`
query hotProducts($pageSize: Int!, $next: Int!) {
  getHotProductBatch(pageSize: $pageSize, next: $next) {next, hasMore, products {_id, images}}
}
`

export default withApollo(class Hot extends React.Component {
...
=======
    ...
    .then(res => {
      // console.log(res)
      if(this.state.next === 1 && res.data.getHotProductBatch.products.length === 0) {
        this.setState({empty: true})
      }else {
        this.setState({
          next: res.data.getHotProductBatch.next,
          hasMore: res.data.getHotProductBatch.hasMore, 
          products: [...this.state.products, ...res.data.getHotProductBatch.products]
        }, () => LayoutAnimation.easeInEaseOut())
      }
    })
    .catch(err => Alert(t(err.graphQLErrors[0].message) || t('somethingWentWrong')))
  ...
  render() {
    return this.state.empty ? <Empty/> : this.state.empty ? <Empty/> : (
      <View style={{flex: 1}}>
